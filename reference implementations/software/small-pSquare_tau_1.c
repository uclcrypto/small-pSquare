...
int Encrypt(const uint8_t *plaintext, const uint8_t* key, const uint8_t* tweak, uint8_t *ciphertext)
{
    uint8_t state[16];
    uint8_t round_tweak[16];
    ...
    {
        // Add tweakey
        ComputeRoundTweakey(round_tweak, key, round_tweakey);
        UpdateRoundTweak(round_tweak);
...
int Decrypt(const uint8_t *ciphertext, const uint8_t* key, const uint8_t* tweak, uint8_t *plaintext)
{
    uint8_t state[16];
    uint8_t round_tweak[16];
    uint8_t round_tweakey[16];
    ...
    for(int i=0; i<r_B; i++)
    {
        // Add tweakey
        ComputeRoundTweakey(round_tweak, key, round_tweakey);
        InverseUpdateRoundTweak(round_tweak);
...
int main()
{
    ...
    uint8_t tweak[16]       = {0x32, 0x39, 0x49, 0x5a, 0x5d, 0x2f, 0x0d, 0x20, 0x70, 0x77, 0x41, 0x5f, 0x5f, 0x5c, 0x16, 0x61};
    uint8_t ciphertext[16];

    if(Encrypt(plaintext, key, tweak, ciphertext))
    {
        for(int i=0; i<15; i++)
        {
    ...
    if(Decrypt(ciphertext, key, tweak, plaintext))
    {
        for(int i=0; i<15; i++)
        {
